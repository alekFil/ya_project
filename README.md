...
Для удаления ветки после объединения используется **git branch -D %%НАЗВАНИЕ_ВЕТКИ%%** или **git branch -d %%НАЗВАНИЕ_ВЕТКИ%%**. Второй вариант удалит ветку только если она была полностью объединена с другой.
...
Также его можно отключить с помощью настройки **merge.ff**: **git config [--global] merge.ff false**
...
С помощью перебазирования ветки возможно добиться состояния fast-forward, даже если раньше было не-fast-forward.

## Подходы к работе с ветками - workflow 

**Feature branch workflow** — простой и самый популярный вариант. Если коротко, в нём для каждого нового изменения создаётся новая ветка, которая позже вливается в main с помощью git merge. **В результате main всегда состоит из merge-коммитов**.
...
10. Удалить текущий привязанный origin - **$ git remote rm origin # эта команда удалит текущий origin**; добавить новый origin - **$ git remote add origin git@github.com:%ВАШ_АККАУНТ%/github-case.git**
11. Работа с механизмом Pull request описана в файле Pull_request.pdf. Перед созданием нового пул-реквеста считается хорошей практикой перейти в главную ветку, «подтянуть» в неё изменения, а затем добавить эти изменения в вашу ветку с помощью git merge main.